...
export class PlaceCardComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.arrayAssessmentStars = Array(Math.round(this.placeAssessment))
      .fill(0)
      .map((x, i) => i);
    ...
    if (this.placeDescription.length > maxLength) {
      this.placeDescription = this.placeDescription
  ...
  selectPlace() {
    ...
    this._router.navigate([
      "/tienda/" + this._formatService.formatString(this.placeName),
    ]);
  }
}
