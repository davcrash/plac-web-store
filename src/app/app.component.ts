...
export class AppComponent {
  ...
  componentAdded(event) {
    ...
    if (event.isInSellIn) {
      //si esta en el componente de vende en plac
    ...
    if (event.showElements) {
      ...
      if (event.sideBarMode) {
        //si esta en el home emite un evento true para que el navbar no se sobreponga
        this._opened = true;
        this._sideBarMode = "push";
        this._closeOutside = false;
      } else {
        //si no esta en el home se sobrepone
        this._opened = false;
  ...
  manageScroll() {
    if (this.isInSellIn) {
      this._sellInService.setScrollValue(
        this.contentPages.nativeElement.scrollTop
      );
    }
  }
