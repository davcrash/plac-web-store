...
export class ProductModalComponent implements OnInit {
  ...
  ngOnInit() {
    this.uid = localStorage.getItem("user_data")
      ? JSON.parse(localStorage.getItem("user_data")).plac_user_id
      : "";
    ...
    this._route.params.subscribe((routeParam) => {
      ...
      this._productModalService.getProductById(this.productId).subscribe(
        (res) => {
          this.product = res;
          ...
          if (!routeParam.name) {
            ...
            let url = this._router
              .createUrlTree(
                [this._formatService.formatString(this.product.product_name)],
            ...
            if (
              this.product.product_name.trim().toLowerCase() !=
              this._formatService.unformatString(routeParam.name).toLowerCase()
            ) {
              let queryParams = this._route.snapshot.queryParams;
              let url = this._router
                .createUrlTree(
                  [
                    "../" +
                      this._formatService.formatString(
          ...
          if (this.product.product_description.length > maxLength) {
            this.productShortDescription = this.product.product_description
          ...
          this.productInCart = this._shopCartService.getProductInCart(
            this.product
          );
  ...
  ngAfterViewInit() {
    setTimeout(() => {
      //el timeOut es para evitar un error
  ...
  redirectCloseModal() {
    ...
    try {
      url = this._router
        .createUrlTree(["../../../"], { relativeTo: this._route, queryParams })
        .toString()
        .replace(/%20/g, " ");
    } catch (error) {
      url = this._router
  ...
  viewMoreText() {
    this.product.product_description === this.productViewDescription
      ? (this.productViewDescription = this.productShortDescription)
      : (this.productViewDescription = this.product.product_description);
  }
  ...
  sendNewQuestion() {
    if (this.uid) {
      ...
      this._productModalService.sendNewQuestion(this.question).subscribe(
        (data) => {
  ...
  addUnits() {
    if (this.quantityToAddToCart <= 50) {
      ...
      this.productInCart.totalProduct =
        this.product.product_price * this.quantityToAddToCart;
    }
  }
  ...
  addProductToCart() {
    this._shopCartService.addProductToCart(
      this.product,
  ...
  shareProduct(destination) {
    ...
    switch (destination) {
      case "facebook":
        window.open(
          `http://www.facebook.com/sharer.php?u=${this.product.product_dynamic_link}`,
          "facebookwindow",
      ...
      case "pinterest":
        window.open(
          `http://www.pinterest.com/pin/create/bookmarklet/?url=${this.product.product_dynamic_link}&media=${this.product.product_images[0].url}&is_video=false&description=${this.product.product_description}`,
          "pinterestwindow",
          "toolbar=0, status=0, width=650, height=650"
        );
        break;
      case "twitter":
        window.open(
          `https://twitter.com/intent/tweet?url=${
            this.product.product_dynamic_link
          }&text=${this.product.product_description.replace(
            "%",
            "%25"
          )}&via=placapp&`,
          "twitterwindow",
