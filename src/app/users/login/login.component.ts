...
export class LoginComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if (localStorage.getItem('user_data')) {//si el usuario esta logueado
    ...
    this._loginService.manageLogIn(placAuth, this.deviceInfo)
      .subscribe(result => {
        ...
        if (result.message == "user_not_exist") {
          ...
          ngZone.run(() => {
            routerService.navigate(['register'], { queryParams: dataToRegister, skipLocationChange: true });
          });
  ...
  async phoneLogin() {
    ...
    await this._loginService.loginWithPhoneNumber(phoneWithCountry, this.recaptchaVerifier)
