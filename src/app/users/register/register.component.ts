...
export class RegisterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this._route.queryParams.subscribe((result) => {
      ...
      this.plac_user.plac_user_name = this.user.displayName
        ? this.user.displayName
        : "";
      this.plac_user.plac_user_email =
        this.additionalUserInfo.profile != undefined
          ? this.additionalUserInfo.profile.email
          : "";
      this.plac_user.plac_user_image = this.user.photoURL
        ? this.user.photoURL
        : "empty";
      this.createForm();
    });
  ...
  get f() {
    return this.placUserForm.controls;
  }
  ...
  submitForm() {
    ...
    if (this.placUserForm.status == "VALID") {
      ...
      this.placUserForm.value.plac_user_image = this.user.photoURL
        ? this.user.photoURL
        : "empty";

      this._registerService
        .registerPlacUser(
          this.placUserForm.value,
          this.authentication,
          this.installation
        )
        .subscribe(
            ...
            if (result.status == "success") {
              ...
              if (result.message === "23000") {
                swal({
                  title: "Oops...",
                  text:
                    "Este correo ya esta registrado en plac deseas anclarlo a la cuenta ya existente",
                  icon: "info",
                ...
                swal(
                  "Oops...",
                  "Algo anda mal por favor inténtalo después",
                  "error"
                );
              }
            }
