...
*/
  ...
  ngOnInit() {
    this._localStorageService.watchStorage().subscribe((data) => {
      ...
      if (data.change === "shop-cart") {
        this.objectPlace = JSON.parse(localStorage.getItem("shop-cart"))[
          this.indexPlace
        ];
        if (!this.objectPlace) {
          location.reload();
    ...
    this._purchaseService.getCountries().subscribe(
      (result) => {
      ...
      (error) => {
        ...
        swal(
          "Opps..",
          "Ocurrio un error obteniendo los departamentos",
          "error"
        );
      }
    );
    ...
    this._purchaseService.getDepartments().subscribe(
      (result) => {
  ...
  deleteAddress(address, index) {
    ...
    swal({
      ...
      text: `Se eliminara la dirección ${
        address.city.country_id != "COL"
          ? JSON.parse(address.plac_user_address).mainWay
      ...
      if (value === true) {
        this._purchaseService
          .deleteAddress(address.plac_user_shipping_address_id)
          .subscribe(
            (result) => {
  ...
  getPaymentMethodsAndCityPrice(placeId, cityId, productType) {
    ...
    this._purchaseService.getPaymentMethodsAndCityPrice(request).subscribe(
      (result) => {
      ...
      (error) => {
        ...
        swal(
          "Opps..",
          "Ocurrio un error obteniendo los metodos de pago",
          "error"
        );
      }
    );
  ...
  addUnit(index) {
    ...
    this.objectPlace.total = this._shopCartService.calculateTotalPlace(
      this.objectPlace
    );
    this.objectPlace.order_detail[index].totalProduct =
      quantity * parseFloat(product.product_price);
  ...
  getUserAdresses() {
    this._purchaseService.getUserAddresses(this.uid).subscribe(
      (data) => {
        ...
        if (this.userAddresses.length > 0) {
          ...
          this.addressSelected.city.country_id == "COL"
            ? (this.currentAddressFormatted = this.addressSelected.address_formatted)
            : (this.currentAddressFormatted = JSON.parse(
                this.addressSelected.plac_user_address
              ).mainWay);
        } else {
  ...
  selectAddres(address) {
    if (!this.deleteIsActive) {
      ...
      this.couponText = "";
      this.getPaymentMethodsAndCityPrice(
        this.objectPlace.place.place_id,
        address.city_id,
        "product"
      );
  ...
  addNewAddress() {
    ...
    this._purchaseService.addNewAddress(this.addressModel).subscribe(
      (response) => {
        this.getUserAdresses();
  ...
  selectToEditAddress() {
    ...
    if (this.countrySelected == "COL") {
      this.departmentSelected = this.addressSelected.city[
        "city_department/state"
      ];
      this.selectDepartment();
    }
    this.addressModel.city_id = this.addressSelected.city["city_id"];

    this.addressModel.plac_user_address = JSON.parse(
      this.addressSelected.plac_user_address
    );
  ...
  editAddress() {
    ...
    this.addressModel.plac_user_id = this.uid;
    this._purchaseService
      .editAddress(this.addressIdToEdit, this.addressModel)
      .subscribe(
        (response) => {
          this.getUserAdresses();
          this.shippingFormSubmitted = false;
  ...
  buyOrder() {
    ...
    let orderModel = {
      ...
      order_resumed: {
        ...
        //cambiar los temp
        subTotal_tmp: this.couponInfo.coupon
          ? this.couponInfo.coupon.resumed.subTotal
          : this.objectPlace.total,
        shipping_price_tmp: this.couponInfo.coupon
          ? this.couponInfo.coupon.resumed.shipping_price
          : this.shippingPrice,
      },
    ...
    this._purchaseService.createOrder(order).subscribe(
      (response) => {
  ...
  selectDepartment() {
    ...
    this._purchaseService
      .getCityByFilter(this.countrySelected, this.departmentSelected)
      .subscribe(
        (result) => {
  ...
  selectCountry() {
    ...
    if (this.countrySelected != "COL") {
      ...
      this._purchaseService.getCityByFilter(this.countrySelected).subscribe(
        (result) => {
          this.cities = result.data;
        },
        (error) => {
